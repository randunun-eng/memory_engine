...
| 2026-04-18 | consolidator/serve | `consolidator-not-scheduled-in-http-serve`: The memory-engine FastAPI app (`src/memory_engine/http/app.py`) starts `/v1/ingest` and `/v1/recall` routes but never schedules `core.consolidator.consolidation_pass()` as a background task. In the live twincore-alpha deployment as of 2026-04-18 this manifests as 42 ingested events and 0 neurons in the engine DB — `/v1/recall` runs cleanly but returns empty because there's nothing to rank. The twin-agent still drafts replies (falling back on identity+profile prompt), but there is no memory retrieval happening end-to-end, which invalidates Phase 7 P1 #4 (eval baseline) until scheduled. Root cause: Phase 2 shipped `consolidation_pass()` as a pure function expecting a caller; Phase 6.5 wired up the HTTP routes but did not add a FastAPI `lifespan` that starts a consolidation loop. Also requires: (a) embed_fn loaded from sentence-transformers at startup, (b) PolicyDispatch pointed at an LLM (Gemini or local Ollama), (c) per-persona loop cadence (suggest 60s between passes). | **Phase 7 P0 #4**: Added `consolidator_lifespan` in `src/memory_engine/http/lifespan.py` wired via `FastAPI(lifespan=...)`; one background task iterates all personas per tick (default 60s) calling `consolidation_pass()`; MiniLM loaded once at startup; LLM backend is Google AI Studio (default `gemma-4-31b-it`, see `consolidator-gemma-4-baseline-invalidated`); signing keys sourced from env (see `consolidator-ai-studio-shared-key`). Metric `wiki_v3_consolidator_lag_seconds{persona=<id>}` updates each tick with the age of the oldest unconsolidated message event. | Resolved |
| 2026-04-18 | consolidator/keys | `consolidator-ai-studio-shared-key`: The consolidator emits rule-8 neuron-mutation events (`operator_action` payloads for neuron_created / supersede) and needs an Ed25519 keypair to sign them. There is no `owner_public_key` column on `personas` yet (see earlier DRIFT `create_persona-inline-SQL`), so the lifespan reads a single keypair from env — `MEMORY_ENGINE_CONSOLIDATOR_PRIVATE_KEY_B64` + `MEMORY_ENGINE_CONSOLIDATOR_PUBLIC_KEY_B64` — and uses it for every persona. `append_event` only checks the signature against the public key it's passed, so this is self-consistent at write time, but it means every consolidation event across every persona on this deployment is signed by the same key. When per-persona owner keys land (Phase 7+ identity-signature work), replace with a lookup that selects the persona's owner pubkey. | Phase 7+ backlog: persist `owner_public_key` on `personas` at creation time, store encrypted owner private keys in `secret_vault`, swap the lifespan to fetch per-persona keys. Blocks multi-persona deployments where persona owners are distinct humans. | Open |
| 2026-04-20 | retrieval/baseline | `retrieval-baseline-p1-4`: P1 #4 eval baseline captured on frozen fixture corpus. **MRR@10 = 0.975, P@5 = 0.240 (ceiling-bounded), Hit@10 = 20/20.** Stack: BM25Plus + sqlite-vec (paraphrase-multilingual-MiniLM-L12-v2 384-dim) + graph, fused via RRF. Corpus: 30 neurons across 6 clusters (electronics, spouse, hobbies, bees, self_facts, domain_facts). Queries: 20, phrased in a mix of English / Singlish / typos. Relevance labels: LLM-generated (`gemini-2.5-flash` via `build_eval_relevance.py`) — most queries have 1 relevant neuron, four have 2. Harness: `tests/eval/test_retrieval_baseline.py`, run `uv run pytest tests/eval/test_retrieval_baseline.py --eval -v -s`. Re-run to record regressions when ranker or embedder changes. | P@5 shows the expected mathematical ceiling (0.2 for 1-relevant queries, 0.4 for 2-relevant) — switch to Hit@K + NDCG@10 in a future harness bump for more interpretable numbers. Consider adding adversarial queries (phrasings that should return EMPTY) to measure precision against irrelevant retrieval. | Resolved |
| 2026-04-20 | grounding/phase7-remeasurement | `grounding-phase7-remeasurement`: Formal re-measurement of the grounding gate against the Phase 7 production stack (gemini-2.5-flash + paraphrase-multilingual-MiniLM-L12-v2 + per-event-max similarity), replacing the Phase 2 baseline of 72% (BoW + concat + 0.40). Ran 50-fixture threshold sweep (`tests/eval/test_grounding_accuracy.py::test_grounding_threshold_sweep`). Results: threshold 0.25 → 60% acc (precision 60%, recall 100% — accepts ALL ungrounded, below embedder noise floor); 0.50 → 72% acc (matches Phase 2 via different mechanism); **0.60 → 88% acc (+16pp over Phase 2, precision 87.5%, recall 93.3%)**; 0.70 → 82% (over-filters). Similarity distribution: grounded mean 0.82 (range 0.56-0.98), ungrounded mean 0.57 (range 0.25-0.89), clean 0.25 separation. Swept full 0.00-0.95 range. This settles the grounding-gate baseline for P1 #4 eval. | Ship threshold 0.60 as new default (was 0.25 which was wildly wrong for L12). Update `_consolidation_loop` + docker-compose.yml env. Live neurons produced under 0.25 threshold are likely higher false-positive rate — P1 #5 quarantine/neuron review should use the new 0.60 cut when re-assessing. | Resolved |
| 2026-04-18 | consolidator/llm | `consolidator-gemma-4-baseline-invalidated`: Consolidator LLM default switched from `ollama/llama3.1:8b` (the model the Phase 2 grounding-accuracy baseline of 72% was measured against) to `gemma-4-31b-it` via Google AI Studio, because the twincore-alpha deployment has no Ollama service and `gemini-2.5-flash` shares a 15 RPM free-tier pool with twin-agent's drafting. Gemma-family quota on AI Studio is a separate pool, solving the rate-limit split. But this is a re-measurement event for the grounding gate — the 72% number on 50 fixtures no longer applies. | Phase 7 pre-eval: re-run `tests/eval/test_grounding_accuracy.py` against `gemma-4-31b-it` with real MiniLM embeddings, record the new baseline in the existing `grounding/phase2` DRIFT entry. Must happen before P1 #4 eval baseline capture — otherwise the MRR@10 number can't be attributed to the retrieval stack alone. | Open |
| 2026-04-18 | consolidator/dedup | `consolidator-duplicate-extraction-loop`: Live smoke discovered consolidator creates exact-content duplicate neurons. Root cause: when `working_memory` entries decay below activation threshold and get pruned, `_find_unconsolidated_events` picks those events up again on the next tick and re-runs extraction. Contradiction detection doesn't catch it because the LLM judges duplicate-vs-duplicate as "complement", not "contradict". Live evidence: 10 distinct claims → 102 neurons with most content repeated 10×. Downstream damage: BM25Okapi's negative-IDF floor collapses `idf[harsha] = 0.0` once a token appears in >50% of docs, so `/v1/recall` returns empty even on trivial keyword matches. Surgical fix shipped: `_promote_candidate()` does a pre-insert lookup by `(persona_id, kind, content_hash, counterparty_id)` and — if an active neuron already exists — reinforces it (bumps `distinct_source_count`, merges `source_event_ids`) instead of INSERTing. Restores BM25 health and matches the Phase 2 rule-15 discipline (distinct sources count; repetitions don't inflate). | **Phase 7 followup**: also stop the re-extraction loop at its source — events promoted to working_memory should not re-enter extraction when WM prunes them. Options: (a) add a `consolidated_events` set table that survives WM pruning, (b) record a `consolidated_at` column on events, (c) mark events as consolidated via an event flag. (a) is the cleanest additive migration. | Open |
| 2026-04-18 | db/sqlite-vec | `sqlite-index-corruption-during-live-writes`: Live smoke on twincore-alpha hit `PRAGMA integrity_check` = "Tree 27 page 27: btreeInitPage() returns error code 11, wrong # of entries in index ix_working_memory_event" after ~15 minutes of concurrent consolidator writes (working_memory + neurons + rule-8 operator_action events) and sqlite3 CLI reads. Recoverable via `sqlite3 .recover` dump + restore; data tables intact, only a working_memory index was damaged. Suspected cause: sqlite-vec virtual-table ops interacting poorly with WAL + concurrent readers, or the consolidator's tight insert/commit loop. Reproduced once on 2026-04-18; need to monitor. | **Phase 7 immediate**: add a daily `PRAGMA integrity_check` to the consolidator loop (log + alert on failure); wrap `.recover` + restore as a runbook step. **Phase 7+ backlog**: either (a) serialize writes through a single asyncio task owning the connection, (b) use a writer queue, or (c) migrate to Postgres for production deployments. Incident doc: `data/incidents/2026-04-18-wm-index-corruption.md` (TODO). | Open |
| 2026-04-18 | grounding/concat-over-citation | `grounding-concat-over-citation`: The grounding gate computes similarity as `cosine(embed(candidate), embed(concat(all_cited_events)))`. This is wrong under a common extractor behaviour: when the extractor is given a batch of 16 events and emits single-sentence claims, it frequently over-cites — listing the full batch as `source_event_ids` even though only one event supports the claim. The concat-embedding then averages out the signal, and real-world extractions score 0.10–0.25 cosine even when grounding is perfect. Live evidence (2026-04-18 twincore-alpha, 99 events): 44/44 candidates quarantined `low_similarity` at threshold 0.40, including `"Harsha did 4cf alone"` — a verbatim English translation of event 4's Sinhala text. Pragmatic shim: `MEMORY_ENGINE_CONSOLIDATOR_SIMILARITY_THRESHOLD` env var, default lowered to 0.25 for the Phase 7 MVP path. | Proper fix (Phase 7 post-MVP): replace the concat with `max_i cosine(embed(candidate), embed(events[i]))` — scoring against the BEST-matching single source event rather than the averaged bag. Either (a) change `_concatenate_event_text` to return per-event chunks and iterate, or (b) have the extractor emit tighter citations (prompt-level change). (a) is safer; (b) is subject to model variance. Re-record baseline after the fix. | Open |
| 2026-04-18 | deploy/dockerfile | `consolidator-dockerfile-branch-pin`: `twincore-alpha/memory_engine_service/Dockerfile` clones memory_engine from `--branch phase-6.5-http-surface`, even though that branch merged to `main` (P1 #6, commit `5ba2733`). The P0 #4 consolidator fix lands on `main`, so the container won't pick it up until either (a) the Dockerfile is rebased to `--branch main`, or (b) the commits are also pushed to `phase-6.5-http-surface`. | Operator: flip Dockerfile to `--branch main` on the next twincore-alpha rebuild. Safe — branch is merged, identical commits. | Open |
