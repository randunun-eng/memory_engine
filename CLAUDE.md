...
| P1 #4: eval baseline | UNBLOCKED — awaiting grounding re-measurement | P0 #4 code is in; before capturing the MRR@10/P@5 baseline, re-run `tests/eval/test_grounding_accuracy.py` against `gemini-2.5-flash` + MiniLM + per-event-max (commit `8355414` changed the gate) so the retrieval number is attributable to the ranker, not a different grounding gate. DRIFT `consolidator-gemma-4-baseline-invalidated` still captures the model-switch rationale. Design choices Q1/Q2/Q3 still open. |
