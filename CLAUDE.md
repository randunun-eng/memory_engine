...
| **P0 #4 (NEW): consolidator not scheduled in HTTP serve** | **PENDING** | Discovered 2026-04-18 while scoping P1 #4. Live engine has 42 events, 0 neurons, 0 quarantine — `core.consolidator.consolidation_pass()` is never called by the FastAPI app. `/v1/recall` runs clean but returns empty. Twin-agent still drafts (identity+profile fallback) but no memory is retrieved end-to-end. Fix: FastAPI `lifespan` spawns per-persona 60s consolidation loop; requires embed_fn + PolicyDispatch wired. DRIFT: `consolidator-not-scheduled-in-http-serve`. **Blocks P1 #4.** |
| P1 #4: eval baseline | BLOCKED on P0 #4 | 100 queries + expected top-k + MRR@10 ≥ 0.6, P@5 ≥ 0.7. Cannot capture baseline until neurons exist. Design choices (Q1/Q2/Q3) also still open. |
| P1 #5: first quarantine review | N/A YET (0 quarantined) | Quarantine table is empty because consolidator never ran. Revisit after P0 #4. |
