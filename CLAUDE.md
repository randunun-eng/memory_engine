...
| P1 #4: eval baseline | **CAPTURED 2026-04-20** | **MRR@10 = 0.975, NDCG@10 = 0.978, Hit@5 = 1.000, Hit@10 = 1.000, P@5 = 0.240 (ceiling-bounded).** Frozen 30-neuron corpus + 20 real queries + 5 adversarial queries (known-empty relevance) + LLM-generated relevance labels in `tests/fixtures/eval_{neurons,queries,relevance}.yaml`. Harness: `tests/eval/test_retrieval_baseline.py`. Relevance-label regenerator: `tests/eval/build_eval_relevance.py` (one-shot, requires `GEMINI_API_KEY`). Grounding-gate baseline: threshold 0.60 → 88% accuracy (DRIFT `grounding-phase7-remeasurement`). 19/20 real queries hit rank 1; q05 ("what language does babi prefer") hit rank 2. Beats CLAUDE.md §9 acceptance ≥ 0.6 MRR@10 by 0.375. Design: seed corpus + LLM-labeled + frozen snapshot (all three Q1/Q2/Q3 recommendations). Adversarial queries surface a real architectural observation: retrieval always returns SOMETHING — filtering non-matches is the caller's concern (twin-agent prompt builder should threshold on fused score before injecting). Stack: BM25Plus + vector (MiniLM-L12) + graph, RRF fusion. |
