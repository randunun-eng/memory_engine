...
- **Current phase:** Phase 3 (Invariants + Healer) — **COMPLETE**
- **Phase 3 acceptance criteria:** All met. 84/84 tests pass (17 Phase 3 integration + 13 Phase 3 invariant + prior phases). ruff + mypy clean. All 16 governance rules have registered invariant checks (meta-test passes). Rules 12 (3 checks), 14 (2 checks), 15 (2 checks) have multiple checks covering different attack vectors. Critical violations trigger halt. Warnings log to healing_log. Auto-repair functions for provenance (quarantine) and distinct_source_count (correct). Halt mechanism: durable halt_state table (survives restart) + in-memory flag (immediate). Healer loop runs every 60s via asyncio.create_task. Rule 1 trigger existence verified via sqlite_master query (schema drift defense). Scan baseline: 3ms at 50 events + 10 neurons, ~0.6s extrapolated to 10k (30s budget).
- **Last worked on:** 2026-04-16. Implemented: migration 003 (healing_log + halt_state), healing/invariants.py (21 checks across 16 rules), healing/checker.py (InvariantChecker with full and critical-only scan), healing/repair.py (repair_missing_provenance, repair_distinct_count_mismatch), healing/halt.py (durable HaltState with halt_state table, engage_halt, release_halt, load_halt_state, assert_not_halted), healing/loop.py (healer_loop background task, start_healer_task).
- **Next session target:** Phase 4 — Identity + Counterparties. Start with `migrations/004_identity.sql`, then `src/memory_engine/identity/persona.py`.
- **Blocked by:** Nothing.
- **Decisions since last update:** (1) Invariant checks use raw SQL exclusively — no domain helper imports, independent failure modes from the code they check. (2) Halt mechanism uses halt_state table (durable, survives restart) + in-memory flag (immediate). On process startup, load_halt_state() reads the table. Does not yet append 'halted' event to events table (deferred to Phase 5 when signing keys available). (3) Halt release never deletes events — sets halt_state.active=0, marks healing_log entries resolved. (4) Rule 7 (retrieval async) is a structural placeholder — real enforcement tested in Phase 1 invariant tests. (5) Rule 13 (pillar hierarchy) is a structural placeholder — enforcement lives in retrieval lens layer. (6) Tests that need to violate CHECK constraints use `PRAGMA ignore_check_constraints` to simulate corruption scenarios. (7) S608 (SQL injection warnings) suppressed for invariants.py — WHERE clauses built from hardcoded strings, not user input. (8) Healer loop uses asyncio.create_task with 60s interval; scan that exceeds interval logs a warning (overlapping scans are a bug).
- **Notes:** 21 checks across 16 rules. Rules 12, 14, 15 have the most coverage (3, 2, 2 checks respectively) per attack-surface-proportional budgeting. Scan duration baseline: 3ms at current fixture size, pessimistic 0.6s at 10k events (50x headroom vs 30s budget; see DRIFT.md). Phase 6 adds `--perf` flag test with real 10k-event database for precise measurement.
