...
- **Current phase:** Phase 2 (Consolidator + Grounding Gate) — **COMPLETE**
- **Phase 2 acceptance criteria:** All met. 15/15 Phase 2 tests pass (10 integration + 5 invariant). 51/51 total (Phase 0 + Phase 1 + Phase 2). ruff + mypy clean. Grounding gate rejects unresolved citations and low-similarity candidates. Quarantine receives rejected candidates. Echo citations do not inflate distinct_source_count (mem0 audit). Validity times never default to now (rule 16). Prompt cache includes persona_id in key (pitfall 6). All LLM calls go through dispatch (policy plane invariant).
- **Last worked on:** 2026-04-16. Implemented: migration 002 (working_memory, quarantine_neurons, episodes, prompt_templates), policy plane (dispatch.py, registry.py, broker.py, cache.py), core consolidator (consolidator.py, extraction.py, grounding.py, contradiction.py). Added new exceptions (PromptNotFound, DispatchError, LLMResponseParseError, GroundingRejection).
- **Next session target:** Phase 3 — Invariants + Healer. Start with `migrations/003_invariants.sql`, then `src/memory_engine/healing/invariants.py`.
- **Blocked by:** Nothing.
- **Decisions since last update:** (1) Policy dispatch uses mock LLM backend in tests — real LLM integration deferred to Phase 7 first-user. (2) Grounding gate similarity check uses pure-Python cosine similarity, not sqlite-vec, to keep the gate independent of the DB. (3) Contradiction detection uses keyword overlap heuristic for candidate filtering before LLM judge — avoids sending every neuron to the LLM. (4) Consolidator runs promote → reinforce → decay → prune in strict sequence per pass. (5) Prompt registry loads from filesystem markdown files with YAML frontmatter. (6) Context broker validates and filters params per site to prevent prompt injection via unexpected fields. (7) Working memory decay uses exponential half-life model (configurable, default 30 min).
- **Notes:** Consolidation pass uses mock LLM in tests. Phase 7 will exercise with real Ollama/LiteLLM backend. Grounding similarity_threshold=0.40 is provisional — track quarantine rate when running with real data and adjust. The consolidator's contradiction check runs the LLM judge twice for the same pair (once pre-insert, once post-insert for supersession) — optimize in Phase 6.
