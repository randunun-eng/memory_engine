...
- **Blocked by:** Nothing.
- **Decisions since last update:** (1) Invariant checks use raw SQL exclusively — no domain helper imports, independent failure modes from the code they check. (2) Halt mechanism uses in-memory flag (immediate) + healing_log (durable); does not yet append 'halted' event to events table (requires signing keys, deferred to Phase 5). (3) Halt release never deletes events — appends resolution, doesn't clear the halt record. (4) Rule 7 (retrieval async) is a structural placeholder — real enforcement tested in Phase 1 invariant tests. (5) Rule 13 (pillar hierarchy) is a structural placeholder — enforcement lives in retrieval lens layer. (6) Tests that need to violate CHECK constraints use `PRAGMA ignore_check_constraints` to simulate corruption scenarios. (7) S608 (SQL injection warnings) suppressed for invariants.py — WHERE clauses built from hardcoded strings, not user input.
- **Notes:** 21 checks across 16 rules. Rules 12, 14, 15 have the most coverage (3, 2, 2 checks respectively) per attack-surface-proportional budgeting. The checker runs all checks in a single scan — no periodic loop yet (Phase 6 adds the background task with 60s interval). Scan performance untested at 10k events (needs --perf flag test, deferred to Phase 6 operational hardening).
