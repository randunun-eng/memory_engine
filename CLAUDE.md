...
| P1 #4: eval baseline | **CAPTURED + EXPANDED 2026-04-21** | **50 real queries on 90-neuron corpus: MRR@10 = 0.953, NDCG@10 = 0.897, Hit@5 = 1.000, Hit@10 = 1.000, Zero-recall = 0/50.** First baseline (30 neurons / 20 queries, 2026-04-20) was MRR 0.975 / NDCG 0.978 — small-N bias. Expanded corpus adds 4 clusters (Sri Lanka travel, memory-engine project facts, Sri Lankan food, AI/ML domain) plus episodic project events. Every query surfaces a relevant neuron in the top 5 — retrieval is sound. Grounding baseline: similarity-only 88% @ 0.60 (DRIFT `grounding-phase7-remeasurement`); full pipeline with LLM judge 94% accuracy, 100% precision (DRIFT `grounding-full-pipeline-with-judge`). Harness: `tests/eval/test_retrieval_baseline.py` + `test_grounding_accuracy.py`. Fixtures: `tests/fixtures/eval_{neurons,queries,relevance}.yaml`. Adversarial queries (5) confirm retrieval always returns SOMETHING — filtering non-matches is the caller's concern (twin-agent prompt builder should threshold on fused score). Stack: BM25Plus + vector (MiniLM-L12) + graph, RRF fusion. |
